...
const ManageListinng = () => {
  ...
  const niches=['lifestyle', 'fitness', 'food', 'travel', 'tech', 'gaming', 'fashion', 'beauty', 'business', 'finance', 'education', 'entertainment', 'music', 'art', 'sports', 'health', 'other']
  ...
  const handleSubmit = async(e)=>{
    ...
    try {
      if(isEditing){
        ...
        const {data}= await api.put('/api/listing', formDataInstance, {headers:{
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <Section title='Basic Information'>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <SelectField 
            label='Niche/Category *' options={niches}
        ...
        <Section title='Account Metrics'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
            ...
            />
            <SelectField 
            label='Primary Audience Age Range' options={ageRanges}
...
const SelectField = ({label, options, value, onChange, required= false})=>{
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <select value={value} onChange={(e)=>onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300'
      required={required}
    >
        <option value=''>
          Select...
...
const TextAreaField = ({label,  required=false, value})=>(
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <textArea rows={5} value={value} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' required={required}
    />
  </div>
)

export default ManageListinng
